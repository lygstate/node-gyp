...
var fs = require('graceful-fs')
  ...
  , isIojs = parseInt(process.versions.node) >= 1
  , distLibName = isIojs ? 'iojs' : 'node'
...
function install (gyp, argv, callback) {
  ...
  var distUrl = gyp.opts['dist-url'] || gyp.opts.disturl || 'http://' + (isIojs ? 'iojs' : 'nodejs') + '.org/dist'
  ...
  function go () {
    ...
    mkdir(devDir, function (err, created) {
      ...
      var tarballUrl = tarPath ? tarPath : distUrl + '/v' + version + '/' + distLibName + '-v' + version + '.tar.gz'
        , badDownload = false
        , extractCount = 0
      ...
      function afterTarball () {
        ...
        if (win) {
          // need to download [iojs|node].lib
          async++
          var libDownloader = isIojs ? downloadIojsLib : downloadNodeLib
          libDownloader(deref)
        }
